...
#' @param survey default is "goa", change if age comps from multiple fisheries
...
#' @examples
#'
q_ts_saa <- function(year, survey = "goa", afsc_species, area, afsc, save = TRUE){

  .files <- grep(paste0(survey,"_ts_saa"),
  ...
  if(isTRUE(save)){
    sql_run(afsc, .one) %>%
      write.csv(here::here(year, "data", "raw", paste0(survey, "_ts_saa_age_data.csv")),
                row.names = FALSE)
    sql_run(afsc, .two) %>%
      write.csv(here::here(year, "data", "raw", paste0(survey, "_ts_saa_length_data.csv")),
                row.names = FALSE)
  } else {
