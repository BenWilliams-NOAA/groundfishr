...
lls_length_comp <- function(YEAR, species = NULL, rec_age, lenbins = NULL){
  ...
  read.csv(here::here(year, "data", "raw", "lls_length_summary_data.csv")) %>%
  ...
  read.csv(here::here(year, "data", "raw", "lls_length_comp_data.csv")) %>%
    dplyr::rename_all(tolower) %>%
    dplyr::filter(area_code > drop[[1]],
                  !(area_code %in% drop[[2]]),
                  country != drop[[3]],
  ...
  write.csv(size_comp, here::here(year, "data", "output", "lls_length_comp.csv"),
            row.names = FALSE)
